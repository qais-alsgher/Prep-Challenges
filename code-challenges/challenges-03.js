...
var aa= [[]];
let a=0;
function newArray(name,tech){
aa[a]={"fullName":name ,"tech":tech};
a++;
};
    ...
    for(let i=0;i<arr.length;i++){
        if(arr[i]["yearsOfExperience"]>1 && arr[i]["firstName"]!=null)
            if(arr[i]["lastName"]===null){
                newArray(arr[i]["firstName"],arr[i]["tech"]); }
              else{
                  const fullName=arr[i]["firstName"]+" "+arr[i]["lastName"];
                   newArray(fullName,arr[i]["tech"]);}
